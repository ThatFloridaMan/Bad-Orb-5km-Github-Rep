...
class FD_WallGroupContainer
{
	...
	void PrepareWallGroups(notnull array<ref WallLengthGroup> groups, bool forward, string middleObj)
	{
		...
		if (!middleObj.IsEmpty())
			m_fMiddleObjectLength = FD_WallGeneratorEntity.MeasureEntity(middleObj, forwardAxis);
		...
		foreach (WallLengthGroup group : groups)
		{
			...
			foreach (WallWeightPair pair : group.m_aWallPrefabs)
			{
				...
				wallLength = FD_WallGeneratorEntity.MeasureEntity(wallPair.m_sWallAsset, forwardAxis);
	...
	//------------------------------------------------------------------------------------------------
	// constructor
	void FD_WallGroupContainer(notnull array<ref WallLengthGroup> items, bool forward, string middleObj)
	{
